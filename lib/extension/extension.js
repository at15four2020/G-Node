...
exports.Extension = class Extension extends EventEmitter {
    ...
    run() {
        ...
        this.#gEarthExtensionServer.connect(this.#port);

        this.#gEarthExtensionServer.on('connect', () => {
            console.log("Connected to G-Earth "+this.#port);
        });

        this.#gEarthExtensionServer.on('error', err => {
            switch(err.code) {
                case "ECONNREFUSED":
                    this.emit('error', new Error("Connection to G-Earth refused, make sure G-Earth is active"));
                    return
            }

            this.emit('error', new Error("Unknown connection error"));
        });

        this.#gEarthExtensionServer.on('close', () => {
            console.log("G-Earth "+this.#port+" connection closed");
            // process.kill(0);
        });
    ...
    #modifyMessage(hMessage) {
        ...
        if(this.#packetInfoManager) {
            ...
            let packetNames = [...new Set(packetInfos.map(p => p.getName()))];
            let packetHashes = [...new Set(packetInfos.map(p => p.getHash()))];
